...
export interface Asset { id: string; title: string; type: string; summary: string; status: "IDEA" | "DRAFT" | "PRODUCTIZED" | "PUBLISHED" | "MONETIZED" | "ARCHIVED"; price: number; format: string; salesCopy: string; createdAt: string; updatedAt: string; }
...
export interface DemoDb { opportunities: Opportunity[]; offers: Offer[]; contentItems: ContentItem[]; assets: Asset[]; decisions: Decision[]; briefings: Briefing[]; lifestyle: LifestyleItem[]; tasks: Task[]; }
