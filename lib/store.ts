...
const store = getStore("personal-empire-os", { consistency: "strong" });
...
export async function addOffer(input: { name: string; audience: string; problem: string; promise: string; pricingModel: string; priceMin: number; priceMax: number; ctaUrl: string; }) {
  ...
  const item = {
    ...
    ctaUrl: input.ctaUrl,
    deliverables: ["Discovery", "Framework", "Executive memo"],
    createdAt: now(),
    updatedAt: now()
  };
...
const DEFAULT_SETTINGS: EmpireSettings = {
  empireName: "Personal Empire",
  ownerName: "",
  currency: "AED",
  timezone: "Asia/Dubai",
  primaryMarket: "",
...
export async function generateWeeklyBrief() {
  ...
  const item = {
    ...
    weekObjective: "Turn one high-value idea into a monetizable asset and publish two authority signals.",
    topMoves: [
