...
export default async function LeadsPage() {
  ...
  // Build share links for the header
  ...
  return (
    <AppShell title="Leads" subtitle="Every inquiry that came through your offer and asset pages.">
      <div className="grid gap-6">
        ...
        {leads.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-[1.25rem] border border-border bg-surface">
            <div className="grid grid-cols-[2fr_1.5fr_1fr_1fr] gap-3 border-b border-border px-5 py-4 text-xs uppercase tracking-wide text-muted">
              <div>Lead</div>
              <div>Source</div>
              <div>Status</div>
              <div>Date</div>
            ...
            {leads.map(lead => (
              <div key={lead.id} className="grid grid-cols-[2fr_1.5fr_1fr_1fr] gap-3 border-b border-border/70 px-5 py-4 last:border-b-0">
                <div>
